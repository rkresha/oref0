...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions) {
    ...
    if (basaliob > max_iob) {
		...
		// setting high threshold for communication transmittion - https://github.com/bewest/decoding-carelink/issues/116 - Setting this so the loop doesn't fail if sets max basal over 6.35
		var dexcomMax = 6.39;
//        var maxSafeBasal = Math.min(profile.max_basal, 3 * profile.max_daily_basal, 4 * basal);
        
        //var maxSafeBasal = tempBasalFunctions.getMaxSafeBasal(profile);
        var maxSafeBasal = dexcomMax;
        ...
        var insulinScheduled = currenttemp.duration * (currenttemp.rate - basal) / 60;
        if (insulinScheduled >= insulinReq * 1.2) { // if current temp would deliver >20% more than the required insulin, lower the rate
            rT.reason += currenttemp.duration + "m@" + (currenttemp.rate - basal).toFixed(3) + " = " + insulinScheduled.toFixed(3) + " > req " + insulinReq + "+20%. Setting temp basal of " + rate + "U/hr";
            return tempBasalFunctions.setTempBasal(rate, 30, profile, rT, currenttemp);
        }
